...
- [x] **Evaluator tuning enforcement** — shipped in `[Unreleased]`. Triggers: (a) `hooks/scripts/session-load.js` SessionStart nudge when ≥3 HUMAN-VERDICT files disagree with Val since last `agents/val-evaluator.md` patch; (b) `/forge-disagree` UX surface for writing the artifacts. The tuning *patch* itself remains a deliberate human-in-the-loop activity per the harness paper warning against auto-tuning evaluators against themselves.
- [x] **Calibration-read enforcement** — shipped in `[Unreleased]`. `hooks/scripts/val-calibration.js` (PreToolUse:Task) prepends `references/calibration-examples.md` as a `<calibration-anchor>` block to every val-evaluator Task spawn. The calibration is now structural, not procedural. Disable with `OHMYCLAUDE_HOOK_VAL_CALIBRATION=off`.
...
- [x] **`db_state` real backend** — shipped in `[Unreleased]`. DSN-based dispatch to `sqlite3` / `psql` / `mysql` CLI via `OHMYCLAUDE_DB_DSN`, with `$N` parameter substitution + post-substitution safety re-check, configurable timeout, row cap, and output redaction for known secret patterns. `scripts/test-probe.js` covers 5 `db_state` paths + 1 `http_probe` round-trip; CI installs `sqlite3` and runs the smoke suite with `OHMYCLAUDE_TEST_OFFLINE=1`.
...
- [x] **HUMAN-VERDICT UX** — shipped in `[Unreleased]`. New slash command `/forge-disagree <test-id>` (`commands/forge-disagree.md`) reads Val's TEST artifact, prompts via `AskUserQuestion` for verdict / divergence / heuristic-gap, writes `HUMAN-VERDICT-<id>.md` in the format documented in `skills/evaluator-tuning/SKILL.md:43-68`. The session-load tuning-due nudge surfaces when 3+ disagreements have accumulated.
...
- [x] **Extended secret-redaction surface** — shipped in `[Unreleased]`. Pattern count 4 → 9: added GitHub fine-grained PATs, Slack tokens (xoxa/xoxb/xoxp/xoxr/xoxs), JWTs, `Authorization: Bearer|Basic` header values, Google API keys. Test-probe assertion 5b confirms JWT redaction end-to-end.
...
- [x] **Hardcoded tuning threshold** — shipped in `[Unreleased]`. `OHMYCLAUDE_TUNING_THRESHOLD` env var (default 3) controls the disagreement count that triggers the nudge.
- [x] **Watermark fragility on fresh install** — shipped in `[Unreleased]`. `readWatermarkMs()` in `session-load.js` now prefers `.claude/.ohmyclaude/last-val-patch.iso` (ISO content first, mtime fallback) and only falls back to plugin-file mtime when the sentinel is absent. Documented in `skills/evaluator-tuning/SKILL.md` Step 3 — the user writes the sentinel after applying a Val prompt patch.
