...
- **Calibration enforcement hook** — `hooks/scripts/val-calibration.js` (PreToolUse:Task). Detects `@val-evaluator` Task spawns and prepends `skills/write-contract/references/calibration-examples.md` as a `<calibration-anchor>` block to `tool_input.prompt`, making the calibration-read a structural part of every val-evaluator invocation rather than a procedural expectation in `agents/val-evaluator.md:174`. Per-hook toggle: `OHMYCLAUDE_HOOK_VAL_CALIBRATION=off`. Registered in `hooks/hooks.json` and `manifests/install-modules.json` (hooks-quality, default profile).
- **`/forge-disagree <test-id>`** slash command — `commands/forge-disagree.md`. Reads `.claude/pipeline/TEST-<id>.md`, prompts via `AskUserQuestion` for verdict / divergence / heuristic-gap, writes `.claude/pipeline/HUMAN-VERDICT-<id>.md` in the schema documented at `skills/evaluator-tuning/SKILL.md:43-68`. Also surfaces a tuning-due nudge when ≥3 disagreements have accumulated since the last Val patch.
