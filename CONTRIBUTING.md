...
| Nature | Strategy |
|---|---|
| **Methodology / knowledge** — prompts, checklists, decision frames, named personas | **Embed inline** as `skills/<name>/SKILL.md`. Pin upstream version in frontmatter. Include a `## Attribution` block. Add a contract test (`scripts/test-*-fallback.js`) if the invariant matters. Examples: SuperClaude knowledge-skills (`sc-spec-panel`, `sc-brainstorm`, `sc-research`); ohmyclaude-original methodology skills (`write-contract`, `evaluator-tuning`). |
| **Executable tool** — CLI, server, binary, anything the model should *call* | **Wrap as MCP** in `.claude-plugin/.mcp.json` (the plugin-shipped file). For trivial wrappers, write a **stdlib-only** stdio server under `scripts/mcp-servers/<name>.js` (~200 LOC is typical). For pre-packaged MCP servers that already speak MCP, declare stdio/http directly. Register as an install module. Examples: `ohmyclaude-fs` (wraps `tree`, stdlib Node) and `code-review-graph` (external Python MCP via `uvx`). Dev-personal MCP servers (your own web-search, API-token-bearing endpoints) belong in **user scope** — `~/.claude/settings.json` or `~/.claude/mcp/` — never in the repo. The repo root `/.mcp.json` is gitignored for this reason. |
...
4. Add to `.claude-plugin/.mcp.json` (plugin-shipped) with a `description` field.
5. Register an install module in `manifests/install-modules.json`; include in `default` and/or `power` profiles.
...
4. No sub-section headers like `### Changed — <subtopic>`. One `### Changed` per version.
...
| **Sprint contracts** *(v3.0.0+)* | `skills-contract` | @paige-product (drafts), @val-evaluator (signs/grades) | `write-contract`, `evaluator-tuning` |
| **SuperClaude knowledge** | `skills-superclaude` | Specific agents at well-defined triggers | `sc-spec-panel`, `sc-brainstorm`, `sc-research` |
| **Session lifecycle** *(opt-in, deregistered v2.6.0)* | `skills-session` | User via /save and /load | `save`, `load` |

Pipeline artifact schemas (`SDD-*`, `CODE-REVIEW-*`, `REVIEW-*`, `UX-SPEC-*`) live inline in each consuming agent's prompt as of v2.6.0 — the four `write-*` skills that previously held them were retired.
